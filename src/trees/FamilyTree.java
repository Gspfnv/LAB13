...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException ("Invalid format: " + line);
		...
		String childrenString = line.substring(colonIndex + 1);
				//The substring of line that starts just after colonIndex and 
				//goes through the end of the line. You'll use a different version of substring().
		String[] childrenArray = childrenString.split(",");
				//Call childrenString.split(). Check the API for details. 
				//The result will be an array of strings, with the separating commas thrown away.
		...
		else
		{
			...
			if (parentNode == null) 
			{
				throw new TreeException("Parent is not found: " + parent);
			}
